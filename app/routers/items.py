...
def create_item_endpoint(item_in: ItemCreate, db: Session = Depends(get_db)) -> ItemResponse:
    if item_exists_by_sku(db, item_in.sku):
        logger.warning(
            "😞 Item could not be created because the SKU already exists operation=create_item sku=%s",
            item_in.sku,
        )
...
def get_item_endpoint(item_id: int, db: Session = Depends(get_db)) -> ItemResponse:
    ...
    if item is None:
        logger.warning("😞 Item was not found operation=get_item item_id=%s", item_id)
...
def update_item_endpoint(
    ...
    if existing_item is None:
        logger.warning("😞 Item was not found operation=update_item item_id=%s", item_id)
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item not found")

    if item_in.sku is not None and item_exists_by_sku(db, item_in.sku, exclude_item_id=item_id):
        logger.warning(
            "😞 Item could not be updated because the SKU already exists operation=update_item item_id=%s sku=%s",
            item_id,
            item_in.sku,
        )
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="SKU already exists")
...
def delete_item_endpoint(item_id: int, db: Session = Depends(get_db)) -> Response:
    ...
    if item is None:
        logger.warning("😞 Item was not found operation=delete_item item_id=%s", item_id)
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Item not found")
